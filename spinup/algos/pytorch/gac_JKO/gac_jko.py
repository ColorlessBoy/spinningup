...
import gym
import time
import spinup.algos.pytorch.gac_JKO.core as core
from spinup.utils.logx import EpochLogger

from geomloss import SamplesLoss
# from torch.utils.tensorboard import SummaryWriter

# writer = SummaryWriter('~/Github/spinningup/log')
...
class ReplayBuffer:
    ...
    def store(self, obs, act, rew, next_obs, done):
        ...
        self.obs_square_mean = self.obs_square_mean / self.total_num * (self.total_num - 1) + np.array(
            obs) ** 2 / self.total_num
...
def gac_jko(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
         ...
         beta_sh=1, eta=100, sh_p=2, blur_constraint=1, scaling=0.95, backend="tensorized",
         std=1.0, epsilon_limit=5.0):
    """
    ...
    sinkhorn_divergence_con = SamplesLoss(loss="sinkhorn", p=sh_p, blur=blur_constraint, backend=backend,
                                          scaling=scaling,
                                          debias=True)
    ...
    def compute_loss_pi(data, beta_pi, beta_sh):
        ...
        epsilon = (std * torch.randn(o2.shape[0], epsilon_dim, device=o2.device)).clamp(-epsilon_limit, epsilon_limit)
        a2 = ac.pi(o2, epsilon)
        q1_pi = ac.q1(o2, a2)
        q2_pi = ac.q2(o2, a2)
        q_pi = torch.min(q1_pi, q2_pi)

        a2 = a2.view(-1, expand_batch, a2.shape[-1])
        ...
        sh_penalty = 0.0
        if beta_sh > 0.0:
            a4 = ac_targ.pi(o2, epsilon).view(-1, expand_batch, a2.shape[-1])
            sh_penalty = torch.sum(sinkhorn_divergence_con(a2 / act_limit, a4 / act_limit))
        ...
        # Useful info for logging
        if beta_pi > 0.0: mmd_entropy = mmd_entropy.detach().cpu().numpy()
        if beta_sh > 0.0: sh_penalty.detach().cpu().numpy()
        pi_info = dict(pi_penalty=mmd_entropy, sh_penalty=sh_penalty)
    ...
    def update_actor(data, beta_pi, beta_sh, jko_steps=30):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_bias_q', type=float, default=0.0)
    parser.add_argument('--max_beta_pi', type=float, default=0.0)
    parser.add_argument('--max_beta_q', type=float, default=0.0)
    parser.add_argument('--seed', '-s', type=int, default=123)
    parser.add_argument('--epochs', type=int, default=50)
    parser.add_argument('--exp_name', type=str, default='gac_jko')
    ...
    gac_jko(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
         ac_kwargs=dict(hidden_sizes=[args.hid] * args.l),
         gamma=args.gamma, seed=args.seed, epochs=args.epochs,
         logger_kwargs=logger_kwargs, max_bias_q=args.max_bias_q,
            max_beta_pi=args.max_beta_pi, max_beta_q=args.max_beta_q)
