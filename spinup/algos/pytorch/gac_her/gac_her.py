...
from spinup.algos.pytorch.gac_her.env_wrapper import HERGoalEnvWrapper
from spinup.algos.pytorch.gac_her.replay_buffer import ReplayBuffer, HindsightExperienceReplayWrapper


def gac_her(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=100, replay_size=int(1e6), gamma=0.99, 
        polyak=0.95, pi_lr=1.0, lr=1e-3, batch_size=100, start_steps=1000, 
        update_after=1000, update_every=50, num_test_episodes=100, max_ep_len=-1, 
        logger_kwargs=dict(), save_freq=1, 
        device='cuda', expand_batch=100, obs_limit=5,
    ...
    """
    Generative Actor-Critic (GAC) with Hindsight Replay Buffer.
    ...
    test_env = HERGoalEnvWrapper(test_env)
    ...
    # Experience buffer
    replay_buffer = ReplayBuffer(obs_dim=obs_dim, act_dim=act_dim, size=replay_size, obs_limit=obs_limit)
    hindsight_replay_buffer = HindsightExperienceReplayWrapper(env, replay_buffer)
    ...
    def get_action(o, deterministic=False):
        ...
        a = ac_targ.act(o, deterministic, noise=noise, obs_limit=obs_limit)
        return a
    ...
    def test_agent():
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                ...
                ep_len += 1
            if(ep_ret > -50.0):
                ep_ret = 1.0 #success, final_reward = 1.0
            else:
                ep_ret = 0.0
    ...
    for t in range(total_steps):
        ...
        hindsight_replay_buffer.store(o, a, r, o2, d, info)
